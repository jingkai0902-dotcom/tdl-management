...
当 Codex 和 Claude 对实现方案有分歧时，不让两个 AI 来回辩论到“谁说服谁”。

固定分工：

| 角色 | 分歧时负责回答的问题 |
|---|---|
| Codex | 这个方案实现成本多大、会改哪些文件、测试怎么做、有没有更小实现路径 |
| Claude | 这个方案有什么可行性风险、边界漏洞、架构后患、测试缺口 |
| 用户 | 基于业务目标、风险承受度和阶段优先级拍板 |

如果 Claude 说“有风险”，Codex 说“能做”，默认回到用户决策，不由 AI 自行扩大范围。

项目文档类调整也适用这条规则：Codex 可以整理和落文档，Claude 负责审风险，用户确认是否进入基线。

### 文档 PR 纪律

本项目的设计文档不是临时笔记。凡是会影响开发方向、阶段优先级、判定口径、协作方式或验收标准的更新，都必须进入 Git PR。

文档 PR 与代码 PR 一样要遵守：

1. 一个 PR 只解决一个清晰主题。
2. PR 描述必须说明本次更新改变了什么决策。
3. 重要文档 PR 需要 Claude review。
4. 用户拍板后再合并。
5. 17 号冻结基线不得直接修改；如需修订，只能新增 V2/V3 文件。

当前项目已经明确：先暂停新增功能开发，按 `18` 和 `19` 对系统板块做重新梳理，再逐个 PR 合并。
...
### 当前项目的默认做法：先组装 review brief，再决定是否使用 UI

截至 2026-05-18，项目规则调整为：

1. Codex 先把 review brief 一次性组装完整。
2. 默认先把可复制的完整 brief 提供给 Frank。
3. 如果 Frank 认为这次值得由 Codex 代发，再明确使用 `Computer Use` 操作 Claude 桌面端。
4. 没有明确确认时，不再把 `Computer Use` 当默认路径。

原因：

1. review brief 的质量关键在“上下文一次性完整”，不是一定要由谁点击发送。
2. `Computer Use` 会引入额外 UI 探索和上下文成本。
3. Claude 桌面端、飞书后台、钉钉后台都属于第三方 UI，后续应只在确实需要时使用。
...
此前项目曾把以下 UI 动作视为可直接代办：

1. 通过 Claude 桌面端发送 PR review 请求并回收结果。
2. 在飞书 / 钉钉后台修改权限设置。
3. 在真实客户端中做交互验证。

从 2026-05-18 起，默认规则更新为：

1. 先判断能否用脚本、命令行、API、数据库或仓库文档完成。
2. 如果确实需要 UI，Codex 先说明：
   - 为什么现在需要 UI。
   - 准备做什么。
   - 不使用 UI 会缺失什么证据。
3. 由 Frank 决定是否继续使用 `Computer Use`。
4. 如果决定使用，只做完成目标所需的最小 UI 操作。
...
为减少重复劳动和不必要的上下文开销，后续默认执行：

1. **脚本优先**
   - 生产部署优先用 `scripts/tdl-deploy-prod.sh`
   - 生产状态检查优先用 `scripts/tdl-prod-check.sh`
2. **命令行优先于 UI**
   - GitHub、ECS、配置、日志、数据库事实先用 CLI / API / SQL 判断
3. **Skill 只承载稳定重复流程**
   - 高复用流程做成 Skill
   - 一次性结论写文档，不把整段历史塞进 Skill
4. **Agent 只为并行，不默认为了省额度**
   - 小任务不用 Agent
   - 只有独立、边界清楚、可并行时才拆
5. **模型选择前置提醒**
   - 机械低风险任务，Codex 先提示可用便宜模型
   - 跨模块判断、生产风险、真实业务边界，Codex 明确建议继续用强模型

详细规则见：

- `24-低耗协作与真实运维工作流-2026-05-18.md`
